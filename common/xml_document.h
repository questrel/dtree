...
public:
  ...
  public:
    ...
    iterator() : obj(NULL), cur(NULL) {}

    // construct iterator with context set to ctx_node
    iterator(xmlXPathContextPtr ctx_, xmlNodePtr ctx_node_)
      : ctx(ctx_), ctx_node(ctx_node_), cur(&ctx_node), last(&ctx_node + 1), obj(NULL)
    {}

    // construct iterator for nodes satisfying XPath expression with context set to ctx_node
    iterator(xmlXPathContextPtr ctx_, xmlNodePtr ctx_node_, const char *xpath_expr)
      : ctx(ctx_), ctx_node(ctx_node_)
    {
      xmlNodePtr save = ctx->node;
      ctx->node = ctx_node_;
    ...
    // non-throwing find()
    iterator find_no_throw(const char *xpath_expr) const throw() {
      return cur ? iterator(ctx, *cur, xpath_expr) : iterator();
    }

    // return iterator for node set satisfying XPath expression with current context
    iterator find(const char *xpath_expr) const {
      iterator retval(find_no_throw(xpath_expr));
      if (retval.cur == NULL)
  ...
  iterator begin(const char *xpath_expr) const {
    return iterator(ctx, NULL, xpath_expr);
  }

  iterator begin(const std::string &xpath_expr) const {
