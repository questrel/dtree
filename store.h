...
#undef BASE_T
  ...
  static void split(const lattice::iterator &ki, int i, const boundary_t &b){
    NOTRACE( std::cerr << __PRETTY_FUNCTION__ << '(' << ki << "," << i << ',' << b << ")\n"; )
    NOTRACE( std::cerr << "root:" << root << '\n'; )
     NOTRACE( std::cerr << "ki.path():" << ki.path() << '\n'; )
...
//exit(1);
...
#else
  {
    ...
    static std::vector<qtl::expr>queries={
      (qtl::expr(qtl::op::column,std::string("1")) <= qtl::expr(qtl::op::lit,qtl::interval("A1"_s))),
      (qtl::expr(qtl::op::column,std::string("2")) <  qtl::expr(qtl::op::lit,qtl::interval("B2"_s))),
      qtl::expr(qtl::op::lit,qtl::interval("B2"_s)) <  qtl::expr(qtl::op::column,std::string("2"))
      &&
      qtl::expr(qtl::op::column,std::string("3")) < qtl::expr(qtl::op::lit,qtl::interval("C3"_s))
      ,
      (qtl::expr(qtl::op::column,std::string("3")) <  qtl::expr(qtl::op::lit,qtl::interval("C3"_s))),
      (qtl::expr(qtl::op::column,std::string("1")) <= qtl::expr(qtl::op::lit,qtl::interval("A1"_s))),
